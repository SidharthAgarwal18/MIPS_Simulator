...
void type_a(string name,int memory[][256],int instruction,int pos)	//for add
{
	int n = name.length();
	int r;
	string reg;
...
int main(int argc,char** argv)
{
	...
	queue<make_pair<int,Node*>> same_row[1024]; 	//an array of queues.
	...
	while(instruction<end_of_instruction && instruction >=0)
	{
		...
		if(type==1) instruction = decode_a(memory_instruction,R,instruction,type,busy,cycle,hash);
