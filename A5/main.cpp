...
Node* memory_manger(Node* head,Node* tail,int N,int buffer_row,bool blocked[],int priority[],int reg_used_when_blocked[][3],pair<int,Node*> busy[][32],bool this_core_finished)
{
	...
	{
		if(reg_used_when_blocked[core_select][0] != -1 && busy[core_select][reg_used_when_blocked[core_select][0]].first==1) return busy[core_select][reg_used_when_blocked[core_select][0]].second;
		if(reg_used_when_blocked[core_select][1] != -1 && busy[core_select][reg_used_when_blocked[core_select][1]].first==1) return busy[core_select][reg_used_when_blocked[core_select][1]].second;
		if(reg_used_when_blocked[core_select][2] != -1 && busy[core_select][reg_used_when_blocked[core_select][2]].first==1) return busy[core_select][reg_used_when_blocked[core_select][2]].second;
	}
	return head->next;
}
	...
	while(cycle <= M && remaining_cores>0)
	{
		...
		for(int I=0;I<N;I++)		//processing each core
		{
			if(cur_instruction[I] >= main_instruction[I] && cur_instruction[I] < exit_instruction[I])	//program has not yet finished
			{
				...
				if((type==1 || type==2 || type==3 || type==4) && write_port[I]!=cycle) cur_instruction[I] = decode_a(memory_instruction,R[I],cur_instruction[I],type,busy[I],cycle,hash,I,0,blocked,priority,reg_used_when_blocked[I],&wait_buffer_size,prev_dram_ins,behind_cycle);
				else if(type==10 && write_port[I]!=cycle) cur_instruction[I] = decode_b(memory_instruction,R[I],cur_instruction[I],type,busy[I],cycle,hash,I,0,blocked,priority,reg_used_when_blocked[I],&wait_buffer_size,prev_dram_ins,behind_cycle);
				...
				else if(type==5 || type==6) cur_instruction[I] = decode_e(memory_instruction,R[I],cur_instruction[I],type,busy[I],cycle,hash,I,0,blocked,priority,reg_used_when_blocked[I],behind_cycle);
				else if((type == 8 || type ==9) && wait_buffer_size<MAX_WAIT_BUFFER_SIZE) 
				{
					if(buffer_row == -1)
					{
						...
						decode_d(memory_instruction,R[I],cur_instruction[I],type,I,exit_instruction[I],busy[I],R_used[I],buffer,blocked,((1024/N)*I*1024),priority,reg_used_when_blocked[I],&wait_buffer_size,prev_dram_ins,behind_cycle);
					...
					else
					{
						...
						if(cur_instruction[I] != decode_d(memory_instruction,R[I],cur_instruction[I],type,I,exit_instruction[I],busy[I],R_used[I],buffer,blocked,((1024/N)*I*1024),priority,reg_used_when_blocked[I],&wait_buffer_size,prev_dram_ins,behind_cycle))
						{
							blocked[I] = false;
				...
				else if(wait_buffer_size==MAX_WAIT_BUFFER_SIZE && (type==8 || type==9))
				...
				// if(prev_instruction==cur_instruction[I]) blocked[I] = true;
