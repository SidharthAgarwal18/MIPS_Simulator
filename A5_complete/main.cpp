...
int main(int argc,char* argv[])
{
	...
	int blocked_hash = -1;									//for best decision during same row
	int blocked_reg = -1;
	...
	unordered_map<int,queue<Node* >> same_row[1024];
	...
	while(cycle <= M && core_remaining.size()!=0)
	{
		...
		if(req_cycle == cycle && buffer_row!=-1)
		{
			...
			{
				int key;
				if(blocked_hash==-1) key	= (*same_row[buffer_row].begin()).first;
				else key = blocked_hash;

				Node* temp = same_row[buffer_row][key].front();
				same_row[buffer_row][key].pop();

				if(same_row[buffer_row][key].size()==0)
				{same_row[buffer_row].erase(key);}
			...
			else if(head->next->data != -1)		//common queue is not empty and buffer_row has no ins.
			{
				...
				int key;
				key = core_of_ins[(temp->data)]>>6 + R_used[temp->data];
				...
				if(temp_core!=-1 && rows_involved_when_blocked[temp_core].size()>0)
				{
					...
					key = (*same_row[buffer_row].begin()).first;
					temp = same_row[buffer_row][key].front();
				}
				int ins = temp-> data;
				prev_dram_ins = ins;
		...
		{	
			...
			if(cur_instruction[I] >= main_instruction[I] && cur_instruction[I] < exit_instruction[I])	//program has not yet finished
			{
				...
				if(type==1 || type==2 || type==3 || type==4) cur_instruction[I] = decode_a(memory_instruction,R[I],cur_instruction[I],type,busy[I],cycle,hash,I,start[(core_of_ins[(cur_instruction[I])])],blocked,rows_involved_when_blocked[I],row_blocking_sw[I],row_blocking_lw[I],priority,&blocked_reg);
				else if(type==10) cur_instruction[I] = decode_b(memory_instruction,R[I],cur_instruction[I],type,busy[I],cycle,hash,I,start[(core_of_ins[(cur_instruction[I])])],blocked,rows_involved_when_blocked[I],row_blocking_sw[I],row_blocking_lw[I],priority,&blocked_reg);
				else if(type==7) cur_instruction[I] = decode_c(memory_instruction,end_of_instruction,cur_instruction[I],cycle,I,start[(core_of_ins[(cur_instruction[I])])],blocked,rows_involved_when_blocked[I],row_blocking_sw[I],row_blocking_lw[I],priority,&blocked_reg);
				else if(type==5 || type==6) cur_instruction[I] = decode_e(memory_instruction,R[I],cur_instruction[I],type,busy[I],cycle,hash,I,start[(core_of_ins[(cur_instruction[I])])],blocked,rows_involved_when_blocked[I],row_blocking_sw[I],row_blocking_lw[I],priority,&blocked_reg);
				else if(type == 8 || type ==9)
				{
					if(buffer_row == -1)
					{
						...
						decode_d(memory_instruction,R[I],cur_instruction[I],type,I,end_of_instruction,busy[I],R_used,buffer,blocked,rows_involved_when_blocked[I],row_blocking_sw[I],row_blocking_lw[I],priority,&blocked_reg);
					...
					else
					{
						...
						if(cur_instruction[I] != decode_d(memory_instruction,R[I],cur_instruction[I],type,I,end_of_instruction,busy[I],R_used,buffer,blocked,rows_involved_when_blocked[I],row_blocking_sw[I],row_blocking_lw[I],priority,&blocked_reg))
						{
							...
							int core_reg_key = (I)>>6 + R_used[(cur_instruction[I])];
							same_row[add/1024][core_reg_key].push(temp);		//pushed in appropriate row.
							cur_instruction[I]++;
				...
				if(blocked[I]==true && blocked_reg>=0 && blocked_reg<32 && blocked_hash==-1 && rows_involved_when_blocked[I].size()==1 && rows_involved_when_blocked[I].find(buffer_row)!=rows_involved_when_blocked[I].end())
				{
					blocked_hash = (I>>6) + blocked_reg;
				}
	...
	while(cycle<=M && empty_dram==false)
	{
		if(req_cycle == cycle && buffer_row!=-1)
		{
			...
			if(same_row[buffer_row].size()>0)
			{	
				int key = (*same_row[buffer_row].begin()).first;
				Node* temp = same_row[buffer_row][key].front();
				same_row[buffer_row][key].pop();
